...
struct Game: Equatable {
    ...
    func reversingLastMove() -> Game {
        ...
        case .enPassant:
            ...
            let capturePosition = DirectedPosition(position: move.destination, perspective: turn.opponent).back!.position
            
            newBoard[capturePosition] = move.capturedPiece
        case .promotion(_):
    ...
    func positionsThreatened(team: Team) -> BooleanChessGrid {
        return zip(board.indices, board)
            .compactMap { position, piece in
                ...
                return piece?.thereatenedPositions(position: position, game: self)
            }
